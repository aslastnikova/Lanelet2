...
void write(const std::string& filename, const LaneletMap& map, const Origin& origin, ErrorMessages* errors, const io::Configuration& params, bool updateVersions) {
  write(filename, map, defaultProjection(origin), errors, params, updateVersions);
}

void write(const std::string& filename, const LaneletMap& map, const Projector& projector, ErrorMessages* errors, const io::Configuration& params, bool updateVersions) {
  ErrorMessages err;
  io_handlers::WriterFactory::createFromExtension(extension(filename), projector, params)->write(filename, map, err, params, updateVersions);
  handleErrorsOrThrow<WriteError>(err, errors);
}

void write(const std::string& filename, const LaneletMap& map, const std::string& writerName, const Origin& origin, ErrorMessages* errors, const io::Configuration& params, bool updateVersions) {
  write(filename, map, writerName, defaultProjection(origin), errors, params, updateVersions);
}

void write(const std::string& filename, const LaneletMap& map, const std::string& writerName,
           const Projector& projector, ErrorMessages* errors, const io::Configuration& params, bool updateVersions) {
  ErrorMessages err;
  io_handlers::WriterFactory::create(writerName, projector, params)->write(filename, map, err, params, updateVersions);
  handleErrorsOrThrow<WriteError>(err, errors);
}
